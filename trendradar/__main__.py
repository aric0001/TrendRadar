...
class NewsAnalyzer:
    ...
    def _prepare_standalone_data(
        ...
        # 加载全局过滤词（GLOBAL_FILTER），从独立展示区排除敏感/不想看的话题
        try:
            _, _, global_filters = self.ctx.load_frequency_words(self.frequency_file)
        except Exception:
            global_filters = []
        ...
        for platform_id in platform_ids:
            ...
            for title, title_data in platform_titles.items():
                # 全局过滤（优先级最高）：排除标题命中的敏感词
                if global_filters and any(gf.lower() in str(title).lower() for gf in global_filters):
                    continue
