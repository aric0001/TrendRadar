...
2. **设置 GitHub Secrets（必需 + 可选平台）**:
   ...
   **GitHub Secret 配置（⚠️ Name 名称必须严格一致）：**
   ...
   - **Secret（值）**：你的飞书机器人 Webhook 地址（该链接开头类似 https://www.feishu.cn/flow/api/trigger-webhook/YOUR_FEISHU_KEY
   <br>
   ...
   **使用方式：**
   ...
   2. **获取推送 URL**：
      - 打开 Bark App
      - 复制首页显示的推送 URL（格式如：`https://api.day.app/YOUR_BARK_KEY
   ...
   **设置步骤：**
   ...
   1. **查看生成的 URL**：
      ...
      - 格式如：`https://hooks.slack.com/services/YOUR_TEAM_ID/YOUR_CHANNEL_ID/YOUR_TOKEN
